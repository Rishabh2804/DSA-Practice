...
//     }
    ...
    Boolean solve(int si, int pi, String s, String p, Boolean[][] dp){
        ...
        if(si >= s.length()){
            // skippable if pattern reperesents star state
            if(pi + 1 < p.length() && p.charAt(pi + 1) == ANY_FREQ) 
                return dp[si][pi] = solve(si, pi + 2, s, p, dp);
        ...
        if(pi + 1 < p.length() && p.charAt(pi + 1) == ANY_FREQ) {
            ...
            // check for match
            if(p.charAt(pi) == ANY_CHAR || (si < s.length() && p.charAt(pi) == s.charAt(si))) {
